...
class GesttaxesejourViewDeclarations extends JView {
  /**
    ...
    $this->app	= JFactory::getApplication();
		...
		if ( $layout == 'edit' ) {
		  $this->document->setTitle(JText::_('COM_GESTTAXESEJOUR_GESTION_TAXE_SEJOUR').' - '.JText::_('COM_GESTTAXESEJOUR_DECLARATIONS_EDIT_DECLARATION'));
		} elseif( $layout == 'create' ) {
      $this->document->setTitle(JText::_('COM_GESTTAXESEJOUR_GESTION_TAXE_SEJOUR').' - '.JText::_('COM_GESTTAXESEJOUR_DECLARATIONS_NEW_DECLARATION'));
    } elseif ( $layout == 'recap' || $layout == 'cheque' || $layout == 'virement' ) {
      if ( $layout == 'recap' ) {
      
      } elseif ( $layout == 'cheque' ) {
        $this->document->setTitle(JText::_('COM_GESTTAXESEJOUR_GESTION_TAXE_SEJOUR').' - '.JText::_('COM_GESTTAXESEJOUR_DECLARATIONS_GESTION_PAIEMENT_CHEQUE'));
      } elseif ( $layout == 'virement' )  {
        $this->document->setTitle(JText::_('COM_GESTTAXESEJOUR_GESTION_TAXE_SEJOUR').' - '.JText::_('COM_GESTTAXESEJOUR_DECLARATIONS_GESTION_PAIEMENT_VIREMENT'));
      }
      $this->document->setTitle(JText::_('COM_GESTTAXESEJOUR_GESTION_TAXE_SEJOUR').' - '.JText::_('COM_GESTTAXESEJOUR_DECLARATIONS_RECAPITULATIF'));
      ...
      if ( $layout == 'recap' ) {
        ...
        $this->app->setUserState( "com_gesttaxesejour.amount", $this->totalDeclarationAfterCom );
        $this->app->setUserState( "com_gesttaxesejour.idtransaction", $this->IDTransaction ); 
      }          
    } else {
      $this->document->setTitle(JText::_('COM_GESTTAXESEJOUR_GESTION_TAXE_SEJOUR').' - '.JText::_('COM_GESTTAXESEJOUR_DECLARATIONS_GESTION_DECLARATIONS'));
    }
