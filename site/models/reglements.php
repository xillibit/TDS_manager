 ...
 * @subpackage	com_contact
 */
class GesttaxesejourModelReglements extends JModel {
  ...
  public function getOwn_hostings() {
    ...
    $query = "SELECT decl.*, hostings.hostingname FROM #__gesttaxesejour_declarations AS decl
              INNER JOIN #__gesttaxesejour_hebergements AS hostings ON decl.hebergement_id=hostings.id 
  ...
  public function getMontantToPay() {
    $user = JFactory::getUser();
    $db = JFactory::getDBO();
    /*$query = "SELECT * FROM #__gesttaxesejour_declarations WHERE declarant_userid={db->quote($user->id)}";
    $db->setQuery((string)$query);
