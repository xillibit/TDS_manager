...
class TdsmanagerAdminModelDeclarations extends TdsmanagerModel {	
  /**
    ...
    $query = "SELECT COUNT(*) FROM #__gesttaxesejour_declarations";
    $db->setQuery((string)$query);
    $total = $db->loadResult();
    
    $this->setState ( 'list.total', $total );
    
    $query = "SELECT decl.id AS decl_id,decl.*, hosting.* FROM #__gesttaxesejour_declarations AS decl
              LEFT JOIN #__gesttaxesejour_hebergements AS hosting ON hosting.id=decl.hebergement_id";
    $db->setQuery((string)$query);
  ...
  public function getListTarifsNuit() {
    $db = JFactory::getDBO();
    $query = "SELECT * FROM #__gesttaxesejour_tarif_nuit";
    $db->setQuery((string)$query);
  ...
  public function getListHebergements() {
    $db = JFactory::getDBO();
    $query = "SELECT hosting.id, hosting.hostingname AS h_name, classement.description AS cl_name FROM #__gesttaxesejour_hebergements AS hosting
              LEFT JOIN #__gesttaxesejour_classements AS classement ON hosting.id_classement=classement.id";
    $db->setQuery((string)$query);
