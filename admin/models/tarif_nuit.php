...
class TdsmanagerAdminModelTarif_nuit extends TdsmanagerModel {	
  /**
    ...
    $value = $this->getUserStateFromRequest ( "com_gesttaxesejour.admin.tarif_nuit.list.limit", 'limit', $this->app->getCfg ( 'list_limit' ), 'int' );
    $this->setState ( 'list.limit', $value );
  
    $value = $this->getUserStateFromRequest ( 'com_gesttaxesejour.admin.tarif_nuit.list.ordering', 'filter_order', 'ordering', 'cmd' );
    $this->setState ( 'list.ordering', $value );
  
    $value = $this->getUserStateFromRequest ( "com_gesttaxesejour.admin.tarif_nuit.list.start", 'limitstart', 0, 'int' );
    $this->setState ( 'list.start', $value );
  
    $value = $this->getUserStateFromRequest ( 'com_gesttaxesejour.admin.tarif_nuit.list.direction', 'filter_order_Dir', 'asc', 'word' );
    if ($value != 'asc')
    $value = 'desc';
    $this->setState ( 'list.direction', $value );
  
    $value = $this->getUserStateFromRequest ( 'com_gesttaxesejour.admin.tarif_nuit.list.search', 'search', '', 'string' );
    $this->setState ( 'list.search', $value );
	} 
  
  public function getTarifNuits() {
    $db = JFactory::getDBO();
    $query = "SELECT COUNT(*) FROM #__gesttaxesejour_tarif_nuit";
    $db->setQuery((string)$query);
    $total = $db->loadResult(); 
    
    $this->setState ( 'list.total', $total );
                                                        
    $query = "SELECT tarif.*, class.description AS classement_name, type.name AS h_type FROM #__gesttaxesejour_tarif_nuit AS tarif
              LEFT JOIN #__gesttaxesejour_classements AS class ON tarif.id_classement=class.id
              LEFT JOIN #__gesttaxesejour_hebergements_type AS type ON tarif.id_hebergement_type=type.id";
  ...
  public function getTarifNuit() {
    ...
    if ( $id > 0 ) {
      $db = JFactory::getDBO();
      $query = "SELECT tarif.*,class.id AS classement_id, type.id AS h_type_id FROM #__gesttaxesejour_tarif_nuit AS tarif
                LEFT JOIN #__gesttaxesejour_classements AS class ON tarif.id_classement=class.id
                LEFT JOIN #__gesttaxesejour_hebergements_type AS type ON tarif.id_hebergement_type=type.id
  ...
  protected function _getUserState() {
    ...
    if ( $id ) {
      $db = JFactory::getDBO();
      $query = "SELECT * FROM #__gesttaxesejour_tarif_nuit WHERE id={$db->quote($id)}";
      $db->setQuery((string)$query);
      $tarif_nuit = $db->loadObject();
  ...
  public function getListClassement () {
    $db = JFactory::getDBO();
    $query = "SELECT * FROM #__gesttaxesejour_classements";
    $db->setQuery((string)$query);
  ...
  public function getListHebergementType() {
    $db = JFactory::getDBO();    
    $query = "SELECT * FROM #__gesttaxesejour_hebergements_type";
    $db->setQuery((string)$query);
