...
class TdsmanagerAdminModelStatistics extends TdsmanagerModel {
  ...
  public function getTauxOccupationStatsCommunes() {
    ...
    $db = JFactory::getDBO();    
    $query = "SELECT hosting.*, decl.*, COUNT(decl.nb_total_personnes) AS pers_occup_total, COUNT(hosting.capacite_personnes) AS personnes_dispo_total, YEAR(decl.date_declarer) AS annee_date_declarer, MONTH(decl.date_declarer) AS mois_date_declarer FROM #__gesttaxesejour_hebergements AS hosting              
              INNER JOIN  #__gesttaxesejour_declarations AS decl ON decl.hebergement_id=hosting.id ".$where." GROUP BY hosting.city";
  ...
  public function getTauxOccupationStatsHeberTypes() {
    ...
    $query = "SELECT hosting.*, type.name AS hosting_type_name, decl.*, COUNT(decl.nb_total_personnes) AS pers_occup_total, COUNT(hosting.capacite_chambres) AS chamb_dispo_total, COUNT(hosting.capacite_personnes) AS personnes_dispo_total FROM #__gesttaxesejour_hebergements AS hosting
              INNER JOIN #__gesttaxesejour_hebergements_type AS type ON hosting.id_hebergement_type=type.id
              LEFT JOIN  #__gesttaxesejour_declarations AS decl ON decl.hebergement_id=hosting.id
              ".$where."
              GROUP BY type.name";
    $db->setQuery((string)$query);
    $tauxoccupationstats = $db->loadObjectlist();
  ...
  public function getTauxOccupationGlobalStats() {
    ...
    $query = "SELECT hosting.*, decl.*, COUNT(decl.nb_total_personnes) AS pers_occup_total, COUNT(hosting.capacite_personnes) AS personnes_dispo_total FROM #__gesttaxesejour_hebergements AS hosting
              LEFT JOIN  #__gesttaxesejour_declarations AS decl ON decl.hebergement_id=hosting.id ".$where;
    $db->setQuery((string)$query);
  ...
  public function getTaxeSejourStats() {
    ...
    $query = "SELECT decl.*, hosting.*, SUM(decl.nb_personnes_assujetties) AS nb_pers_assujetties_total, SUM(decl.montant_encaisse_sejour) AS montant_enc_sejour_total, SUM(decl.duree_sejour) AS duree_sejour_total FROM #__gesttaxesejour_declarations AS decl
  ...
  public function getTaxeSejourGlobalStats() {
    ...
    $query = "SELECT SUM(decl.nb_personnes_assujetties) AS nb_pers_assujetties_total, SUM(decl.montant_encaisse_sejour) AS montant_enc_sejour_total, SUM(decl.duree_sejour) AS duree_sejour_total FROM #__gesttaxesejour_declarations AS decl ".$where;
    $db->setQuery((string)$query);
  ...
  protected function _getQueryCommuneStats($where) {
    $query = "SELECT hosting.*, type.name AS hosting_type_name, decl.*, class.description AS desc_classement, label.nom AS nom_label, users.adress AS adress_owner, users.postalcode AS cp_owner, users.ville AS ville_owner, decl.nb_total_personnes AS pers_occup_total, decl.nb_total_nuitee AS total_nuitee, (decl.nb_total_personnes/hosting.capacite_personnes) AS total_taux_occupation  
              FROM #__gesttaxesejour_hebergements AS hosting 
              INNER JOIN #__gesttaxesejour_hebergements_type AS type ON hosting.id_hebergement_type=type.id
              LEFT JOIN  #__gesttaxesejour_classements AS class ON hosting.id_classement=class.id
              LEFT JOIN  #__gesttaxesejour_hebergements_label AS label ON hosting.id_hebergement_label=label.id
              LEFT JOIN  #__gesttaxesejour_declarations AS decl ON decl.hebergement_id=hosting.id
              LEFT JOIN  #__gesttaxesejour_users AS users ON users.userid=hosting.userid
              ".$where;
    return $query;              
  }
