...
class TdsmanagerAdminControllerHebergements extends TdsmanagerController {
   protected $baseurl = null;
    ...
    $this->app->setUserState( "com_gesttaxesejour.hebergement.id", 0 );    
		...
		$this->setRedirect('index.php?option=com_gesttaxesejour&view=hebergements&layout=create');
	}
	...
	public function save() {
     ...
     if ( !$id ) {      
       ...
       $query = "INSERT INTO #__gesttaxesejour_hebergclass (id_hebergement,id_classement,tarif,userid,id_hebergement_type,id_hebergement_label) 
       ...
       /*$query = "UPDATE #__gesttaxesejour_hebergclass SET id_classement=,tarif=,userid=,id_hebergement_type=,id_hebergement_label= WHERE id_hebergement={$db->quote($id)}";
       $db->setQuery((string)$query);
       $db->Query();
    ...
    if ( $id > 0 ) {
      $db = JFactory::getDBO();
      $query = "DELETE FROM #__gesttaxesejour_hebergements WHERE id={$db->Quote($id)}";
      $db->setQuery((string)$query);
      $db->Query();
    ...
    $db = JFactory::getDBO();
    $query = "UPDATE #__gesttaxesejour_hebergements SET state={$db->Quote($state)} WHERE id={$db->Quote($id)}";
    $db->setQuery((string)$query);
    $db->Query();
