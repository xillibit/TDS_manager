  ...
  /**
	...
	function check() {
		if (empty($this->adress)) {
			$this->setError(JText::_('COM_GESTTAXESEJOUR_REQUIRES_ADRESS'));
			return false;
		}
		
		if (empty($this->postalcode)) {
			$this->setError(JText::_('COM_GESTTAXESEJOUR_REQUIRES_POSTALCODE'));
			return false;
		}
		
		if (empty($this->ville)) {
			$this->setError(JText::_('COM_GESTTAXESEJOUR_REQUIRES_VILLE'));
			return false;
		}
		
		if (empty($this->phone)) {
			$this->setError(JText::_('COM_GESTTAXESEJOUR_REQUIRES_PHONE'));
			return false;
		}
		
		if (empty($this->email)) {
			$this->setError(JText::_('COM_GESTTAXESEJOUR_REQUIRES_EMAIL'));
			return false;
		}
		
		if (empty($this->owner_name)) {
			$this->setError(JText::_('COM_GESTTAXESEJOUR_REQUIRES_OWNER_NAME'));
			return false;
		}
	  ...
	  // Initialise variables.
		...
		if (empty($pks)) {
			...
			// Nothing to set publishing state on, return false.
				...
				$this->setError(JText::_('COM_GESTTAXESEJOUR_NO_ROWS_SELECTED'));
				return false;
			}
		}
